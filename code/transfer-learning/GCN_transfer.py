...
data=np.load(source_path+"\\GraphInput.npy",allow_pickle=True)
adj=np.load(source_path+"\\adj.npy")
...
data=np.load(target_path+"\\GraphInput.npy",allow_pickle=True)
adj=np.load(target_path+"\\adj.npy")
XY=np.load(source_path+"\\XY.npy",allow_pickle=True).T
adj=torch.LongTensor(adj).to(device)
data=torch.FloatTensor(data).to(device)
with torch.no_grad():
    hidden_feature=data
